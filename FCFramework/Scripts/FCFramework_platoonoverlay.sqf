...
waitUntil { !isNil "FCLIB_overlay_groups" };
waitUntil { !isNil "FCLIB_nametag_units" };
	...
	if ( show_nametags ) then {
		{
			...
			if ( _nextunit in (units group player)) then {
				...
				switch ( _nextunit getVariable [ "FCLIB_squad_color", "MAIN" ] ) do {
