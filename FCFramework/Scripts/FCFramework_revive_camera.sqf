revive_ui = compile preprocessFileLineNumbers "FCFramework\scripts\FCFramework_revive_ui.sqf";
...
while { true } do {
	waitUntil { ( player getVariable "FCFramework_isUnconscious") == 1 };
	...
	waitUntil { ((player getVariable "FCFramework_isUnconscious") == 0) || ((player getVariable "FAR_isUnconscious") == 1) };
	closeDialog 0;
	waitUntil {!dialog};
	if ((player getVariable "FCFramework_isUnconscious") != 0) then {
		[] spawn revive_ui;
		waitUntil {dialog};
	};
	waitUntil { ( player getVariable "FAR_isUnconscious"  ) == 0 || !alive player || !dialog };
	player setVariable ["FCFramework_isUnconscious", 0, true];
	closeDialog 5566;
