...
waitUntil { !isNil "FCFramework_view_distance_factor" };
waitUntil { !isNil "FCFramework_force_adjust_view_distance" };
...
while { true } do {
	waitUntil { sleep 0.3;
		...
		|| FCFramework_force_adjust_view_distance };
	...
	if ( _was_vehicle ) then {
		...
		_desired_veh = (round desiredviewdistance_veh) * FCFramework_view_distance_factor;
		...
		_desired_inf = (round desiredviewdistance_inf) * FCFramework_view_distance_factor;
	...
	_desired_obj = (((desiredviewdistance_obj / 100.0) * desiredviewdistance_inf) * FCFramework_view_distance_factor);
