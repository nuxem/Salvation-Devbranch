...
_dialog = createDialog "FCFramework_Menu";
waitUntil { dialog };

if ( FCFramework_allow_customsquads ) then {
	ctrlShow [501, false];
} else {
	ctrlShow [501, true];
...
if ( FCFramework_allow_platoonview ) then {
	ctrlShow [601, false];
} else {
	ctrlShow [601, true];
...
if ( FCFramework_allow_viewdistance ) then {
	ctrlShow [701, false];
...
if ( FCFramework_allow_worldquality ) then {
	ctrlShow [801, false];
} else {
	ctrlShow [801, true];
...
if ( FCFramework_allow_mapmarkers ) then {
	ctrlShow [901, false];
} else {
	ctrlShow [901, true];
...
while { dialog && alive player } do {
	...
	if ( FCFramework_allow_platoonview ) then { ctrlShow [612, show_platoon]; };
	if ( FCFramework_allow_mapmarkers ) then { ctrlShow [912, show_teammates]; };
...
FCFramework_options_profile = [ desiredviewdistance_inf, desiredviewdistance_veh, desiredviewdistance_obj, show_teammates, show_platoon, desired_vehvolume, show_nametags, desired_fps ];
profileNamespace setVariable [ "FCFramework_OPTIONS_PROFILE",FCFramework_options_profile ];
