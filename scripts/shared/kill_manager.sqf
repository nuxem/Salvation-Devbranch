...
if ( isServer ) then {
	...
	if ( side _killer == FCLIB_side_friendly ) then {
		...
		if ( count _nearby_bigtown > 0 ) then {
			combat_readiness = combat_readiness + (0.5 * FCLIB_difficulty_modifier);
			stats_readiness_earned = stats_readiness_earned + (0.5 * FCLIB_difficulty_modifier);
			if ( combat_readiness > 100.0 && FCLIB_difficulty_modifier < 2 ) then { combat_readiness = 100.0 };
		};
	...
	if ( _unit isKindOf "Man" ) then {
		if ( side (group _unit) == FCLIB_side_civilian ) then {
			stats_civilians_killed = stats_civilians_killed + 1;
			if ( isPlayer _killer ) then {
				stats_civilians_killed_by_players = stats_civilians_killed_by_players + 1;

				if ( FCLIB_civ_penalties ) then {
					resources_ammo = resources_ammo - FCLIB_civ_killing_penalty;
					[ [ name _unit, FCLIB_civ_killing_penalty, _killer ] , "remote_call_civ_penalty" ] call BIS_fnc_MP;
				};
			};
		};

		if ( side _killer == FCLIB_side_friendly ) then {
			if ( side (group _unit) == FCLIB_side_enemy ) then {
			...
			if ( side (group _unit) == FCLIB_side_friendly ) then {
