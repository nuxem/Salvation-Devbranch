...
private _skillmodifier = sqrt FCLIB_difficulty_modifier;
private _inVehicle = false;
if ( vehicle _unit != _unit ) then {
	_inVehicle = true;
};

if ( !(FCLIB_autodanger) && (_side == FCLIB_side_friendly)) then {
...
if ( _wounded ) then {
	...
	if ( _side == FCLIB_side_friendly || _side == FCLIB_side_enemy ) then {
