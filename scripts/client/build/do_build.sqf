...
build_confirmed = 0;
_maxdist = FCLIB_fob_range;
_truepos = [];
_debug_colisions = false;

FCLIB_preview_spheres = [];
while { count FCLIB_preview_spheres < 36 } do {
	FCLIB_preview_spheres pushback ( "Sign_Sphere100cm_F" createVehicleLocal [ 0, 0, 0 ] );
};

{ _x setObjectTexture [0, "#(rgb,8,8,3)color(0,1,0,1)"]; } foreach FCLIB_preview_spheres;
	...
	if(buildtype == 1) then {
		...
		if ( buildtype == 8 ) then {
			...
			while { build_confirmed == 1 && alive player } do {
				...
				{
					if ((_x isKindOf "Animal") || ((typeof _x) in FCLIB_ignore_colisions_when_building) || (_x == player) || (_x == _vehicle )) then {
				...
				if ( count _near_objects != 0 ) then {
					FCLIB_conflicting_objects = _near_objects;
				} else {
				...
				if (count _near_objects == 0 && ((_truepos distance _posfob) < _maxdist) && (  ((!surfaceIsWater _truepos) && (!surfaceIsWater getpos player)) || (_classname in boats_names) ) ) then {
					...
					if ( build_invalid == 0 ) then {
						{ _x setObjectTexture [0, "#(rgb,8,8,3)color(1,0,0,1)"]; } foreach FCLIB_preview_spheres;
					};
					_vehicle setpos _ghost_spot;
					build_invalid = 1;
					if(count _near_objects > 0) then {
						FCLIB_ui_notif = format [localize "STR_PLACEMENT_IMPOSSIBLE",count _near_objects, round _dist];
					...
					if( ((surfaceIsWater _truepos) || (surfaceIsWater getpos player)) && !(_classname in boats_names)) then {
						FCLIB_ui_notif = localize "STR_BUILD_ERROR_WATER";
					};
					if((_truepos distance _posfob) > _maxdist) then {
						FCLIB_ui_notif = format [localize "STR_BUILD_ERROR_DISTANCE",_maxdist];
					};
			...
			{ _x setpos [ 0,0,0 ] } foreach FCLIB_preview_spheres;
