...
if ( ( [ getpos player , 500 , FCLIB_side_enemy ] call F_getUnitsCount ) > 4 ) exitWith { hint localize "STR_BUILD_ENEMIES_NEARBY"; };
...
_dialog = createDialog "salvation_build";
waitUntil { dialog };
...
while { dialog && alive player && (dobuild == 0 || buildtype == 1)} do {
	...
	_affordable = false;
	_squad_full = false;
	if ((buildtype == 1) && (count (units group player) >= FCLIB_max_squad_size)) then {
		_squad_full = true;
	};
	_linked = false;
	...
	if (dobuild == 0 && _selected_item != -1 && (_selected_item < (count _build_list))) then {
		...
		if ( buildtype != 8 ) then {
			{ if ( ( _build_item select 0 ) == ( _x select 0 ) ) exitWith { _base_link = _x select 1; _linked = true; } } foreach FCLIB_vehicle_to_military_base_links;
