...
while { true } do {
	{
		if ((_x != group player) && ((side _x == FCLIB_side_friendly))) then {
			if ( (_x in _iconed_groups) && (
				(count units _x == 0) ||  (side _x == FCLIB_side_friendly && (((leader _x) distance (getmarkerpos FCLIB_respawn_marker) < 100) || ((leader _x) distance lhd < 500))))) then {
				clearGroupIcons _x;
			...
			if ( !(_x in _iconed_groups) && (
				...
				if ( side _localgroup == FCLIB_side_friendly ) then {
