...
while { true } do {
...

	{
		private [ "_nextgroup" ];
		_nextgroup = _x;
		if ( (side _nextgroup == FCLIB_side_enemy) && (({ !captive _x } count ( units _nextgroup ) ) > 0)) then {
			if ( [(getpos leader _nextgroup), FCLIB_side_friendly, FCLIB_radiotower_size] call F_getNearestTower != "" ) then {
