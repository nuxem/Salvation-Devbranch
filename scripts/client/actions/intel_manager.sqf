...
while { true } do {
	...
	if (  [ player, 5 ] call F_fetchPermission  ) then {
		...
		_near_intel = (getPosATL player) nearEntities [[FCLIB_intel_laptop, FCLIB_intel_file], 5];
		{
			if ( (captive _x) && !(_x in _actionned_captive_units) && !((side group _x) == FCLIB_side_friendly)  ) then {
				_x addAction ["<t color='#FFFF00'>" + localize "STR_SECONDARY_CAPTURE" + "</t>","scripts\client\actions\do_capture.sqf","",-850,true,true,"","(vehicle player == player) && (side group _target != FCLIB_side_friendly) && (captive _target)"];
				_actionned_captive_units pushback _x;
			};
		} foreach _near_people;

		{
			if ( !(alive _x) || ((player distance _x) > 5) || ((side group _x) == FCLIB_side_friendly) ) then {
				removeAllActions _x;
