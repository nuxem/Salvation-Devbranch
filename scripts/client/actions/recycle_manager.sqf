...
waitUntil { sleep 1; !isNil "FCLIB_all_fobs" };

while { true } do {

	waitUntil { sleep 1; count FCLIB_all_fobs > 0 };
	waitUntil { sleep 1; ( player distance ( [] call F_getNearestFob)) < (2 * FCLIB_fob_range) };
		...
		_detected_vehicles = 	[ (getpos player) nearObjects veh_action_distance ,
							{
								...
								(_x distance ( [] call F_getNearestFob) < FCLIB_fob_range ) &&
								( getObjectType _x >= 8 ) } ]
