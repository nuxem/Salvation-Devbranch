...
_dialog = createDialog "salvation_recycle";
waitUntil { dialog };
...
ctrlSetText [ 132, format [ "%1", round ((_objectinfo select 2) * FCLIB_recycling_percentage) ] ];
...
if ( dorecycle == 1 && !(isnull _vehtorecycle) && alive _vehtorecycle) then {
	[ [ _vehtorecycle,  round ((_objectinfo select 2) * FCLIB_recycling_percentage) ] , "recycle_remote_call" ] call BIS_fnc_MP;
};