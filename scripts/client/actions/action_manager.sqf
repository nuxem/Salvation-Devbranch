...
while { true } do {
	...
	if ( (_fobdistance < _distredeploy || (player distance lhd) < 200) && alive player && vehicle player == player && FCLIB_halo_param > 0 ) then {
	...
	if ( ( player == ( [] call F_getCommander ) || [] call F_isAdmin ) && alive player && vehicle player == player && FCLIB_permissions_param ) then {
	...
	if ( ( count FCLIB_all_fobs > 0 ) && ( FCLIB_endgame == 0 ) && (_fobdistance < _distredeploy || (player distance lhd) < 200) && alive player && vehicle player == player && ( ( [ player, 5 ] call F_fetchPermission ) || ( player == ( [] call F_getCommander ) || [] call F_isAdmin ) ) ) then {
