...
if ( FCLIB_isAtlasPresent ) then {
	...
	_basenamestr = "BASE LINDSEY";
};

while { true } do {
	waitUntil {
		sleep 0.1;
		( FCLIB_force_redeploy || (player distance (getmarkerpos FCLIB_respawn_marker) < 50) ) && vehicle player == player && alive player && !dialog && howtoplay == 0
	};
	...
	if ( !FCLIB_fatigue ) then {
		player enableStamina false;
	};
	player setCustomAimCoef 0.35;
	player setUnitRecoilCoefficient 0.6;

	_dialog = createDialog "salvation_deploy";
	deploy = 0;
	_oldsel = -999;
	...
	while { dialog && alive player && deploy == 0} do {
		...
		for [{_idx=0},{_idx < count FCLIB_all_fobs},{_idx=_idx+1}] do {
			choiceslist = choiceslist + [[format [ "FOB %1 - %2", (military_alphabet select _idx),mapGridPosition (FCLIB_all_fobs select _idx) ],FCLIB_all_fobs select _idx]];
		};
