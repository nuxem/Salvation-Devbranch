...
if ( isNil "FCLIB_last_halo_jump" ) then { FCLIB_last_halo_jump = -6000; };

if ( FCLIB_halo_param > 1 && ( FCLIB_last_halo_jump + ( FCLIB_halo_param * 60 ) ) >= time ) exitWith {
	hint format [ localize "STR_HALO_DENIED_COOLDOWN", ceil ( ( ( FCLIB_last_halo_jump + ( FCLIB_halo_param * 60 ) ) - time ) / 60 ) ];
};

_dialog = createDialog "salvation_halo";
dojump = 0;
...
if ( dojump > 0 ) then {
	FCLIB_last_halo_jump = time;
	halo_position = [ halo_position, random 250, random 360 ] call BIS_fnc_relPos;
	halo_position = [ halo_position select 0, halo_position select 1, FCLIB_halo_altitude + (random 200) ];
	halojumping = true;
	sleep 0.1;
