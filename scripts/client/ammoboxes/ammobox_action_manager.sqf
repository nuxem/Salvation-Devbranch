...
while { true } do {
	...
	if ( [ player, 5 ] call F_fetchPermission ) then {
		...
		{
			_next_truck = _x;
			_truck_load = _next_truck getVariable ["FCLIB_ammo_truck_load", 0];
			...
			if ( !(_next_truck in _managed_trucks) && (_truck_load > 0)) then {
					...
					_next_truck setVariable [ "FCLIB_ammo_truck_action", _action_id, false ];
					_managed_trucks pushback _next_truck;
			};

			if ( (_next_truck in _managed_trucks) && _truck_load == 0 ) then {
				_next_truck removeAction ( _next_truck getVariable ["FCLIB_ammo_truck_action", -1] );
...

		{
			...
			if ( !(_next_box in _managed_boxes) && ( isNull  attachedTo _next_box )) then {
				...
				_next_box setVariable [ "FCLIB_ammo_box_action", _action_id, false ];
...

		{
			...
			if ( !(_next_box in _managed_boxes) || !( isNull  attachedTo _next_box )) then {
				...
				_next_box removeAction ( _next_box getVariable ["FCLIB_ammo_box_action", -1] );
			}
