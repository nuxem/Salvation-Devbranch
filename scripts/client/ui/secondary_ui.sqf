...
if ( isNil "FCLIB_secondary_in_progress" ) then { FCLIB_secondary_in_progress = -1; };

_dialog = createDialog "salvation_secondary";
dostartsecondary = 0;
...
while { dialog && alive player && dostartsecondary == 0 } do {
	...
	_missioncost = FCLIB_secondary_missions_costs select _oldchoice;

	if ( ( _missioncost <= resources_intel ) && ( !FCLIB_secondary_starting ) )  then {
		ctrlEnable [ 103, true ];
