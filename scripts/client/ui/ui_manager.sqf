...
while { true } do {
	...
	if ( _overlayshown ) then {
		...
		((uiNamespace getVariable 'GUI_OVERLAY') displayCtrl (266)) ctrlSetText format [ "%1", FCLIB_ui_notif ];
		((uiNamespace getVariable 'GUI_OVERLAY') displayCtrl (267)) ctrlSetText format [ "%1", FCLIB_ui_notif ];
		...
		if ( _uiticks % 25 == 0 ) then {
			...
			if (!isNil "active_sectors" && ( [] call F_opforCap >= FCLIB_sector_cap)) then {
			...
			_nearest_active_sector = [ FCLIB_sector_size ] call F_getNearestSector;
			if ( _nearest_active_sector != "" ) then {
				_zone_size = FCLIB_capture_size;
				if ( _nearest_active_sector in sectors_bigtown ) then {
					_zone_size = FCLIB_capture_size * 1.4;
				};

				"zone_capture" setmarkerposlocal (markerpos _nearest_active_sector);
				_colorzone = "ColorGrey";
				if ( [ markerpos _nearest_active_sector, _zone_size ] call F_sectorOwnership == FCLIB_side_friendly ) then { _colorzone = FCLIB_color_friendly };
				if ( [ markerpos _nearest_active_sector, _zone_size ] call F_sectorOwnership == FCLIB_side_enemy ) then { _colorzone = FCLIB_color_enemy };
				if ( [ markerpos _nearest_active_sector, _zone_size ] call F_sectorOwnership == FCLIB_side_resistance ) then { _colorzone = "ColorCivilian" };
