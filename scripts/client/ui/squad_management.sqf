...
FCLIB_squadaction = -1;
FCLIB_squadconfirm = -1;
_membercount = -1;
_resupplied = false;
_memberselection = -1;
_selectedmember = objNull;
_dialog = createDialog "salvation_squad";
...
while { dialog && alive player } do {
	...
	if( FCLIB_squadconfirm == 0 ) then {
		FCLIB_squadconfirm = -1;
		FCLIB_squadaction = -1;
	};

	if ( FCLIB_squadconfirm == 1 ) then {
		FCLIB_squadconfirm = -1;
		...
		if ( FCLIB_squadaction == 1 ) then {
			...
			if ( _fobdistance < 100 || count _nearsquad > 0 ) then {
				...
				_tempgmp = createGroup FCLIB_side_friendly;
