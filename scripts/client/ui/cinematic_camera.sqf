...
if ( isNil "FCLIB_all_fobs" ) then { FCLIB_all_fobs = [] };
		...
		if ( !first_camera_round ) then {
			...
			if ( count FCLIB_all_fobs > 0 ) then {
				for [ {_idx=0},{_idx < 2},{_idx=_idx+1} ] do {
					_positions pushback (FCLIB_all_fobs call bis_fnc_selectRandom);
				};
			};
			...
			if ( FCLIB_endgame == 0 ) then {
				 _activeplayers = ( [ allPlayers , { alive _x && ( _x distance ( getmarkerpos FCLIB_respawn_marker ) ) > 100 } ] call BIS_fnc_conditionalSelect );
			...
			if ( showcaminfo && howtoplay == 0 ) then {
				...
				if ( _position distance lhd < 300 ) then {
					...
					if ( _nearest_sector != "" ) then {
						...
						_nearfobs = [ FCLIB_all_fobs, { _x distance _position < 300 } ] call BIS_fnc_conditionalSelect;
						if ( count _nearfobs > 0 ) then {
							_nearest_sector = format [ "FOB %1", military_alphabet select ( FCLIB_all_fobs find ( _nearfobs select 0 ) ) ];
						};
					};
				};
