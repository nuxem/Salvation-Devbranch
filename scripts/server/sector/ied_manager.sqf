...
if ( !(isnull _roadobj) ) then {
	...
	while { _sector in active_sectors && mineActive _ied_obj && !_goes_boom } do {
		_nearinfantry = [ (getpos _ied_obj) nearEntities [ "Man", _activation_radius_infantry ] , { side _x == FCLIB_side_friendly } ] call BIS_fnc_conditionalSelect;
		_nearvehicles = [ (getpos _ied_obj) nearEntities [ [ "Car", "Tank", "Air" ], _activation_radius_vehicles ] , { side _x == FCLIB_side_friendly } ] call BIS_fnc_conditionalSelect;
