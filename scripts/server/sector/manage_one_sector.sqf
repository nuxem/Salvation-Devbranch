...
_infsquad = "militia";
_building_range = 50;
_local_capture_size = FCLIB_capture_size;
_iedcount = 0;
_vehtospawn = [];
_managed_units = [];
_squad1 = [];
...
_popfactor = 1;
if ( FCLIB_unitcap < 1 ) then { _popfactor = FCLIB_unitcap; };
...
if ( (!(_sector in blufor_sectors)) &&  ( ( [ getmarkerpos _sector , [ _opforcount ] call F_getCorrectedSectorRange , FCLIB_side_friendly ] call F_getUnitsCount ) > 0 ) ) then {
	...
	if ( _sector in sectors_bigtown ) then {
		...
		if ( FCLIB_unitcap >= 1) then {
			_squad3 = ([] call F_getAdaptiveSquadComp);
		};
		if ( FCLIB_unitcap >= 1.5) then {
			_squad4 = ([] call F_getAdaptiveSquadComp);
		};
		if((random 100) > (66 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback (militia_vehicles call BIS_fnc_selectRandom); };
		if((random 100) > (50 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback (militia_vehicles call BIS_fnc_selectRandom); };
		if((random 100) > (33 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback ( [] call F_getAdaptiveVehicle ); };
		_spawncivs = true;

		_building_ai_max = round (50 * _popfactor) ;
		_building_range = 110;
		_local_capture_size = _local_capture_size * 1.4;
		_iedcount = (2 + (floor (random 4))) * FCLIB_difficulty_modifier;
	...
	if ( _sector in sectors_capture ) then {
		...
		if((random 100) > (66 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback (militia_vehicles call BIS_fnc_selectRandom); };
		if((random 100) > (33 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback (militia_vehicles call BIS_fnc_selectRandom); };
		_spawncivs = true;
		_building_ai_max = round ((floor (18 + (round (combat_readiness / 10 )))) * _popfactor);
		_building_range = 70;
		_iedcount = (floor (random 4)) * FCLIB_difficulty_modifier;
	...
	if ( _sector in sectors_military ) then {
		...
		if ( FCLIB_unitcap >= 2) then {
			_squad4 = ([] call F_getAdaptiveSquadComp);
		};
		_vehtospawn = [( [] call F_getAdaptiveVehicle ),( [] call F_getAdaptiveVehicle )];
		if((random 100) > (33 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback ( [] call F_getAdaptiveVehicle ); };
		if((random 100) > (66 / FCLIB_difficulty_modifier)) then { _vehtospawn pushback ( [] call F_getAdaptiveVehicle ); };
		_spawncivs = false;
	...
	if ( _sector in sectors_factory ) then {
		...
		_building_ai_max = round ((floor (18 + (round (combat_readiness / 10 )))) * _popfactor);
		_building_range = 100;
		_iedcount = (floor (random 3)) * FCLIB_difficulty_modifier;
	...
	if ( _building_ai_max > 0 && FCLIB_adaptive_opfor ) then {
	...
	if ( _spawncivs && FCLIB_civilian_activity > 0) then {
	...
	while { !_stopit } do {
		...
		if ( ( [_sectorpos, _local_capture_size] call F_sectorOwnership == FCLIB_side_friendly ) && ( FCLIB_endgame == 0 ) ) then {
...

			{
				if (_x isKindOf "Man") then {
					if ( side group _x != FCLIB_side_friendly ) then {
						deleteVehicle _x;
					};
				} else {
			...
			if ( ( [_sectorpos, ( ( [ _opforcount ] call F_getCorrectedSectorRange ) + 300 ), FCLIB_side_friendly ] call F_getUnitsCount ) == 0 ) then {
