...
if ( isNil "FCLIB_military_sectors_already_activated" ) then { FCLIB_military_sectors_already_activated = [] };

if ( !( _sector in FCLIB_military_sectors_already_activated )) then {

	FCLIB_military_sectors_already_activated pushback _sector;

	if ( !FCLIB_passive_income ) then {
		_crates_amount = ceil (((0.5 * FCLIB_sector_military_value) + (random (0.5 * FCLIB_sector_military_value ))) * FCLIB_resources_multiplier);
	...
	if ( count _nearbuildings > 0 ) then {
		...
		if ( count _building_positions >= (_nbintel * 4) ) then {
			...
			while { _nbintel > 0 } do {
				...
				/*
				_tableobject = FCLIB_intel_table createVehicle _buildingposition;
				...
				_chairobject = FCLIB_intel_chair createVehicle _buildingposition;
				...
				*/
				...
				_intelclassname = [ FCLIB_intel_file, FCLIB_intel_laptop ] call BIS_fnc_selectRandom;
				...
				if ( _debug ) then {
					...
					_marker setMarkerColorLocal FCLIB_color_enemy_bright;
					_marker setMarkerTypeLocal "mil_dot";
				};

				/**
				if ( _intelclassname == FCLIB_intel_file ) then {
					_intelobject attachTo [ _tableobject, [0, 0, 0.42] ];
				};

				if ( _intelclassname == FCLIB_intel_laptop ) then {
