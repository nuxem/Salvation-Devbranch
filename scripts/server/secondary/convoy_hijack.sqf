...
{
	_x addEventHandler ["HandleDamage", { private [ "_damage" ]; if ( side (_this select 3) != FCLIB_side_friendly ) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ];
...
private _troops_group = createGroup FCLIB_side_enemy;
...
_convoy_marker setMarkerColor FCLIB_color_enemy_bright;
...

{
	...
	_x setMarkerColor FCLIB_color_enemy_bright;
...
while { _mission_in_progress } do {
	...
	if ( _convoy_attacked && !_disembark_troops) then {
		...
		if (alive (driver _troop_vehicle)) then {
			private _troop_driver_group = (createGroup FCLIB_side_enemy);
