...
_defenders_amount = 15 * ( sqrt ( FCLIB_unitcap ) );
...
_grpdefenders = createGroup FCLIB_side_enemy;
_idxselected = [];
...
_sentry = ceil ((3 + (floor (random 4))) * ( sqrt ( FCLIB_unitcap ) ) );

_grpsentry = createGroup FCLIB_side_enemy;
...
sleep 1;
FCLIB_secondary_in_progress = 0; publicVariable "FCLIB_secondary_in_progress";
...
combat_readiness = round (combat_readiness * FCLIB_secondary_objective_impact);
