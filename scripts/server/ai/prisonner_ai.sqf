...
if ( (!_force_surrender) && (( random 100 ) > FCLIB_surrender_chance) ) exitWith {};

if ( (_unit isKindOf "Man") && ( alive _unit ) && (side group _unit == FCLIB_side_enemy) ) then {
	...
	if ( alive _unit ) then {
		...
		waitUntil { sleep 1;
			!alive _unit || side group _unit == FCLIB_side_friendly
		};
