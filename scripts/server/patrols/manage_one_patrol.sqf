...
while { FCLIB_endgame == 0 } do {
	...
	waitUntil { sleep 0.3; combat_readiness >= (_minimum_readiness / FCLIB_difficulty_modifier); };

	sleep (random 30);

	while {  [] call F_opforCap > FCLIB_patrol_cap } do {
			sleep (random 30);
	};
	...
	if (_is_infantry) then {
		_grp = createGroup FCLIB_side_enemy;
	...
	while { _patrol_continue } do {
		...
		if ( count (units _grp) == 0  ) then {
			...
			if ( time - _started_time > 900 ) then {
				if ( [ getpos (leader _grp) , 4000 , FCLIB_side_friendly ] call F_getUnitsCount == 0 ) then {
					_patrol_continue = false;
					{
	...
	if ( !([] call F_isBigtownActive) ) then {
		sleep (600.0 / FCLIB_difficulty_modifier);
	};
