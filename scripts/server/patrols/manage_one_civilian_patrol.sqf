...
while { FCLIB_endgame == 0 } do {
	...
	_usable_sectors = [];
	{
		if ( ( ( [ getmarkerpos _x , 1000 , FCLIB_side_friendly ] call F_getUnitsCount ) == 0 ) && ( count ( [ getmarkerpos _x , 3500 ] call F_getNearbyPlayers ) > 0 ) ) then {
	...
	if ( count _usable_sectors > 0 ) then {
		...
		_grp = createGroup FCLIB_side_civilian;
		if ( random 100 < 33) then {
			...
			_civveh addEventHandler ["HandleDamage", { private [ "_damage" ]; if (( side (_this select 3) != FCLIB_side_friendly ) && ( side (_this select 3) != FCLIB_side_enemy )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ];
		...
		{ _x addEventHandler ["HandleDamage", { private [ "_damage" ]; if (( side (_this select 3) != FCLIB_side_friendly ) && ( side (_this select 3) != FCLIB_side_enemy )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ]; } foreach units _grp;
		...
		if ( count (units _grp) > 0 ) then {
			if ( count ( [ getpos leader _grp , 4000 ] call F_getNearbyPlayers ) == 0 ) then {
				...
				if ( !(isNull _civveh) ) then {
					 if ( { ( alive _x ) && (side group _x == FCLIB_side_friendly ) } count (crew _civveh) == 0 ) then {
