if ( combat_readiness > 15 ) then {
	...
	_init_units_count = ( ([ getmarkerpos _targetsector , FCLIB_capture_size , FCLIB_side_enemy ] call F_getUnitsCount) );

	if ( !(_targetsector in sectors_bigtown)) then {
		while { (_init_units_count * 0.75) <=  ( [ getmarkerpos _targetsector , FCLIB_capture_size , FCLIB_side_enemy ] call F_getUnitsCount ) } do {
			sleep 5;
		};
	};
	...
	if ( _targetsector in active_sectors ) then {
		...
		_nearestower = [markerpos _targetsector, FCLIB_side_enemy, FCLIB_radiotower_size * 1.4] call F_getNearestTower;

		if ( _nearestower != "" ) then {
			_reinforcements_time = (((((markerpos _nearestower) distance (markerpos _targetsector)) / 1000) ^ 1.66 ) * 120) / (FCLIB_difficulty_modifier * FCLIB_csat_aggressivity);
			if (_targetsector in sectors_bigtown) then {
			...
			if ( (_targetsector in active_sectors) && !(_targetsector in blufor_sectors) && !(_nearestower in blufor_sectors) && (!([] call F_isBigtownActive) || _targetsector in sectors_bigtown)  ) then {
				...
				if ( (random combat_readiness) > (20 + (30 / FCLIB_csat_aggressivity) ) ) then {
